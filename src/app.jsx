...
const autoGenerateDates = async () => {
  ...
  // 1. Jours fériés via l'algorithme (fonctionnel et sûr à 100%)
  ...
  try {
    ...
    // Requête très large sur la zone, on filtrera manuellement dans le code
    const res = await fetch(`https://data.education.gouv.fr/api/records/1.0/search/?dataset=fr-en-calendrier-scolaire&rows=100&refine.zones=${zoneEncoded}`);
    const data = await res.json();
    
    // On récupère les données de l'API si elles existent
    let vacs = data.records
      .filter(r => r.fields.annee_scolaire === `${year1}-${year2}`)
      .filter(r => !r.fields.population || !r.fields.population.toLowerCase().includes("enseignant"))
      .filter(r => r.fields.description && r.fields.description.toLowerCase().includes("vacances"))
      .map(r => {
         const endD = new Date(r.fields.end_date); 
         endD.setDate(endD.getDate() - 1); // La fin est la veille de la rentrée
         const pad = n => String(n).padStart(2, '0');
         return { 
           id: r.recordid, 
    ...
    // Nettoyage des doublons de l'API
    vacs = Array.from(new Map(vacs.map(item => [item.debut, item])).values());

    // 2. PLAN DE SECOURS (Si l'API du Gouvernement est vide/en retard)
