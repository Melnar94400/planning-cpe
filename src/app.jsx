...
const autoGenerateDates = async () => {
    ...
    // Génération mathématique des jours fériés
    ...
    try {
      ...
      // On retire le filtre "population" capricieux de l'URL pour tout télécharger
      const res = await fetch(`https://data.education.gouv.fr/api/records/1.0/search/?dataset=fr-en-calendrier-scolaire&rows=100&refine.annee_scolaire=${year1}-${year2}&refine.zones=${zoneEncoded}`);
      const data = await res.json();
      
      // On filtre les données directement dans le code React (en cherchant "lèves" pour ignorer les accents)
      const vacs = data.records
        .filter(r => r.fields.population && r.fields.population.includes("lèves"))
        .map(r => {
           const endD = new Date(r.fields.end_date); 
           endD.setDate(endD.getDate() - 1); // Fin = veille de la reprise
           const pad = n => String(n).padStart(2, '0');
           return { 
             id: r.recordid, 
